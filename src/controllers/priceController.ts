...
import { DistanceRanges, VenueData, CustomRequest } from '../types';



export const getDeliveryOrderPrice = (req: CustomRequest, res: Response, next: NextFunction) => {
	try {
		const { cart_value, user_lat, user_lon } = req.query;
		const venueData = req.venueData as VenueData;

		const venueCoordinates: [number, number] = venueData.static.venue_raw.location.coordinates;
		const orderMinimumNoSurcharge: number = venueData.dynamic.venue_raw.delivery_specs.order_minimum_no_surcharge;
		const basePrice: number = venueData.dynamic.venue_raw.delivery_specs.delivery_pricing.base_price;
		const distanceRanges: DistanceRanges[] = venueData.dynamic.venue_raw.delivery_specs.delivery_pricing.distance_ranges;
